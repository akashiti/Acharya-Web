...
export default function AdminCategories() {
  ...
  const openEdit = (c) => { setEditing(c); setForm({ name: c.name, description: c.description || '', image: c.image || '', sortOrder: c.sortOrder || 0 }); setShowModal(true); };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) await updateCategory(editing.id, { ...form, slug });
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteCategory(id); fetch(); }
          ...
          <div key={c.id} className="bg-[#1a1a35] rounded-2xl border border-white/5 p-5 group hover:border-purple-500/30 transition-all">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-gray-500 text-xs mt-1">Sort: {c.sortOrder || 0}</p>
              </div>
