...
export default function AdminDashboard() {
  ...
  const stats = [
    { label: 'Total Revenue', value: `₹${data.totalRevenue?.toLocaleString()}`, icon: DollarSign, color: 'from-emerald-500 to-green-600', change: '+12.5%', up: true },
    { label: 'Total Orders', value: data.totalOrders, icon: ShoppingCart, color: 'from-blue-500 to-indigo-600', change: '+8.3%', up: true },
    { label: 'Total Users', value: data.totalUsers, icon: Users, color: 'from-purple-500 to-pink-500', change: '+5.1%', up: true },
            ...
            <tbody>
              {data.recentOrders?.length ? data.recentOrders.map(order => (
                <tr key={order.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="p-4 text-white">{order.user?.name || '—'}</td>
                  <td className="p-4 text-gray-400">{order.orderItems?.length || 0}</td>
