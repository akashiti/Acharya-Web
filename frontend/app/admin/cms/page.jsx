...
export default function AdminCMS() {
  ...
  const fetchData = () => {
    Promise.all([
      api.get('/cms/banners').then(r => setBanners(r.data.data)),
      api.get('/cms/content').then(r => setContents(r.data.data || {})),
  ...
  const openEditBanner = (b) => { setEditingBanner(b); setBannerForm({ title: b.title, subtitle: b.subtitle || '', image: b.image || '', link: b.link || '', position: b.position, active: b.active }); setShowBannerModal(true); };

  const submitBanner = async (e) => {
    e.preventDefault();
    try {
      if (editingBanner) await api.put(`/cms/banners/${editingBanner.id}`, bannerForm);
      else await api.post('/cms/banners', bannerForm);
  ...
  const saveSection = async () => {
    ...
    try {
      ...
      await api.put(`/cms/content/${editingSection}`, { content: parsed });
        ...
        {[{ key: 'banners', icon: Image, label: 'Banners' }, { key: 'content', icon: Type, label: 'Site Content' }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              tab === t.key ? 'bg-purple-600 text-white' : 'bg-[#1a1a35] text-gray-400 hover:text-white border border-white/5'
            }`}>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loading ? [...Array(4)].map((_, i) => (
              <div key={i} className="h-32 bg-[#1a1a35] rounded-2xl animate-pulse" />
            )) : banners.length === 0 ? (
              <p className="text-gray-500 col-span-full text-center py-12">No banners yet</p>
            ) : banners.map(b => (
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${b.active ? 'bg-emerald-500/20 text-emerald-300' : 'bg-gray-500/20 text-gray-400'}`}>
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => deleteBanner(b.id)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg"><Trash2 size={14} /></button>
                  </div>
                </div>
              </div>
            ))}
          ...
          {sectionKeys.map(key => (
            <button key={key} onClick={() => openSection(key)}
              ...
              <p className="text-gray-500 text-sm">
                {contents[key] ? `${Object.keys(contents[key]).length} fields configured` : 'Not configured'}
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-end">
                  <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer pb-2.5">
                    <input type="checkbox" checked={bannerForm.active} onChange={e => setBannerForm({...bannerForm, active: e.target.checked})} className="accent-purple-500" />
                    Active
                  </label>
                </div>
              </div>
          ...
          <div className="bg-[#1a1a35] rounded-2xl border border-white/10 w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 bg-[#0f0f1a] border border-white/10 rounded-xl text-green-300 text-sm font-mono focus:outline-none focus:border-purple-500/50 resize-none"
                spellCheck={false}
              />
            </div>
