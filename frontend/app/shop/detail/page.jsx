            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={handleAdd} disabled={adding || product.stock === 0}
                className={`flex-1 py-3.5 rounded-2xl font-medium text-sm flex items-center justify-center gap-2 transition-all ${
                  product.stock === 0 ? 'bg-sand/30 text-earth/30 cursor-not-allowed' : adding ? 'bg-emerald-500 text-white' : 'bg-plum text-ivory hover:bg-purple hover:shadow-xl'
                }`}>
                <ShoppingBag size={18} />
...
export default function ProductDetailPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-ivory flex items-center justify-center"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-plum"></div></div>}>
