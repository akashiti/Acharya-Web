...
import { BookOpen, PenLine, ShoppingBag, Sparkles, TrendingUp } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    api.get('/journals?limit=1')
      .then((res) => setJournalCount(res.data.pagination?.total || 0))
      .catch(() => {});
    api.get('/orders/my')
    ...
    <div>
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-heading font-bold text-plum">
              Namaste, {user?.name?.split(' ')[0]}!
            </h1>
            <p className="text-earth/50 text-sm font-body">
              Welcome to your spiritual wellness dashboard
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-6 mb-10">
        {quickStats.map(({ icon: Icon, label, value, desc, color, iconColor, href, soon }) => (
          <Link
            ...
            className={`glass-card bg-white/80 p-6 shadow-soft transition-all duration-300 group ${
              soon ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-card hover:-translate-y-1'
            }`}
          >
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <Link
          ...
          className="glass-card bg-white/60 p-6 shadow-soft hover:shadow-card hover:-translate-y-1 transition-all duration-300 group"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-heading font-bold text-plum text-lg">
                Write in Journal
              </h3>
              <p className="text-earth/40 text-sm">
