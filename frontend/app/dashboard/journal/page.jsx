...
import { Plus, PenLine, Trash2, Clock, Smile, Loader2 } from 'lucide-react';
...
export default function JournalListPage() {
  ...
  const fetchJournals = useCallback(async (page = 1) => {
    try {
      setLoading(true);
      const res = await api.get(`/journals?page=${page}&limit=10`);
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-heading font-bold text-plum">
            My Journal
          </h1>
          <p className="text-earth/40 text-sm mt-1">
            {pagination.total} {pagination.total === 1 ? 'entry' : 'entries'}
          </p>
        </div>
        <Link
          href="/dashboard/journal/new"
          className="btn-gold text-sm gap-2 !px-5 !py-2.5"
        >
          <Plus size={16} />
          New Entry
        </Link>
      ...
      {journals.length === 0 && (
        <div className="glass-card bg-white/60 p-12 text-center shadow-soft">
          ...
          <h2 className="text-xl font-heading font-bold text-plum mb-3">
            Your journal awaits
          </h2>
          <p className="text-earth/50 font-body max-w-sm mx-auto mb-8">
            Begin documenting your spiritual journey. Each entry is a step towards
            deeper self-awareness and inner peace.
          </p>
          <Link href="/dashboard/journal/new" className="btn-gold">
      ...
      {journals.length > 0 && (
        <div className="space-y-4">
          {journals.map((journal) => (
            <div
              key={journal.id}
              className="glass-card bg-white/80 p-5 sm:p-6 shadow-soft hover:shadow-card transition-all duration-300 group"
            >
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    {journal.mood && moodEmojis[journal.mood] && (
                      <span className="text-lg" title={journal.mood}>
                  ...
                  <p className="text-earth/50 text-sm line-clamp-2 mb-3">
                    {journal.content}
                  </p>
                  <div className="flex items-center gap-4 text-xs text-earth/30">
                    ...
                    {journal.mood && (
                      <span className="px-2 py-0.5 bg-sand/30 rounded-full capitalize">
                        {journal.mood}
                      </span>
                    )}
                  </div>
                ...
                <button
                  ...
                  className="p-2 rounded-lg text-earth/20 hover:text-red-400 hover:bg-red-50 transition-all flex-shrink-0"
                  title="Delete"
                >
                  {deleting === journal.id ? (
